...
echo "=== Map Data Download ==="
echo "Starting unified download process for OSM, GTFS, and elevation data"
echo "This data will be used by routing engines (Valhalla, GraphHopper, etc.)"

# Configuration with defaults
# Map data directories (shared across routing engines)
OSM_DATA_DIR="$${OSM_DATA_DIR:-/osm_data}"
GTFS_DATA_DIR="$${GTFS_DATA_DIR:-/gtfs_data}"
ELEVATION_DATA_DIR="$${ELEVATION_DATA_DIR:-/elevation_data}"
...
if [ "$${INIT_MODE}" = "check" ]; then
    ...
    # auto or download mode
    if [ ! -d "$${OSM_DATA_DIR}" ]; then
        echo "ERROR: Target directory $${OSM_DATA_DIR} does not exist"
        sleep $${ERROR_SLEEP}
        exit 1
    fi
...
fi  # End of init mode check for GTFS
...
# Check init mode
if [ "$${INIT_MODE}" = "check" ]; then
    echo "Mode: check - verifying elevation data exists..."
    if [ -d "$${ELEVATION_DATA_DIR}" ] && [ "$(ls $${ELEVATION_DATA_DIR}/*.hgt 2>/dev/null | wc -l)" -gt 0 ]; then
        echo "✓ Elevation data found, skipping download"
    else
        echo "✗ Elevation data not found!"
        exit 1
    fi
elif [ "$${INIT_MODE}" = "bypass" ]; then
    echo "Mode: bypass - skipping elevation download"
else
    # auto or download mode
    mkdir -p $${ELEVATION_DATA_DIR}
    cd $${ELEVATION_DATA_DIR}
...
if [ "$${INIT_MODE}" != "bypass" ] && [ "$${INIT_MODE}" != "check" ]; then
...
fi  # End of init mode check for elevation
...
echo "OSM data:"
if [ -d "$${OSM_DATA_DIR}" ] && ls $${OSM_DATA_DIR}/*.pbf >/dev/null 2>&1; then
	pbf_count=$(ls -1 $${OSM_DATA_DIR}/*.pbf 2>/dev/null | wc -l)
	pbf_size=$(du -sh $${OSM_DATA_DIR}/*.pbf 2>/dev/null | tail -1 | awk '{print $1}')
...
echo ""
echo "GTFS feeds:"
if [ -d "$${GTFS_DATA_DIR}" ] && ls $${GTFS_DATA_DIR}/feed_* >/dev/null 2>&1; then
	for feed in $${GTFS_DATA_DIR}/feed_*; do
		if [ -d "$${feed}" ]; then
...
echo ""
echo "Elevation data:"
if [ -d "$${ELEVATION_DATA_DIR}" ] && [ $${downloaded} -gt 0 ]; then
	elev_size=$(du -sh $${ELEVATION_DATA_DIR} 2>/dev/null | awk '{print $1}')
	echo "  $${downloaded} tiles, $${elev_size} total"
else
	echo "  (none)"
fi

# Set ownership and permissions for routing engines (user 1000, group 1000)
echo ""
echo "Setting permissions for routing engines..."
chown -R 1000:1000 $${OSM_DATA_DIR} 2>/dev/null || true
chown -R 1000:1000 $${GTFS_DATA_DIR} 2>/dev/null || true
chown -R 1000:1000 $${ELEVATION_DATA_DIR} 2>/dev/null || true
chmod -R u+rw,g+rw $${OSM_DATA_DIR} 2>/dev/null || true
chmod -R u+rw,g+rw $${GTFS_DATA_DIR} 2>/dev/null || true
chmod -R u+rw,g+rw $${ELEVATION_DATA_DIR} 2>/dev/null || true
