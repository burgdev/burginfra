...
check_osm_data() {
	if ls "$CUSTOM_DIR"/*.pbf >/dev/null 2>&1; then
		local pbf_file
		pbf_file=$(ls "$CUSTOM_DIR"/*.pbf 2>/dev/null | head -n1)
		local size
...
check_gtfs_data() {
	if [ ! -d "$GTFS_DIR/feed*" ]; then
		echo "✗ GTFS feeds directory does not exist"
		return 1
	fi

	local file_count=$(find "$GTFS_DIR" -type f 2>/dev/null | wc -l)
...
check_elevation_data() {
	if [ ! -d "$CUSTOM_DIR/elevation_data" ]; then
		echo "✗ Elevation data directory does not exist"
		return 1
	fi

	local hgt_count=$(find "$CUSTOM_DIR/elevation_data" -name "*.hgt" 2>/dev/null | wc -l)
...
check_valhalla_config() {
	if [ ! -f "$CUSTOM_DIR/valhalla.json" ]; then
...
check_valhalla_tiles() {
	if [ ! -f "$CUSTOM_DIR/valhalla_tiles.tar" ]; then
		echo "✗ valhalla_tiles.tar not found"
		return 1
	fi

	local size=$(du -h "$CUSTOM_DIR/valhalla_tiles.tar" | cut -f1)
